...
class AdvisorQuickLinkController extends Controller
{
    ...
    public function index(Request $request){        
        ...
        $params = [
            ...
            'dataTableUrl'      => Null,
            'create'            => AccessController::checkAccess("advisor_quick_link_create") ? route('advisor_quick_link.create') : false,
    ...
    public function store(Request $request){
        try{
            ...
            if( $request->id == 0 ){
                ...
                $this->saveActivity($request, "Add new advisor quick link ");
            }else{
                $message = 'Quick Link\'s updated successfully';
                $data = $this->getModel()->find($request->id);
                $data->updated_by = $request->user()->id;
                $this->saveActivity($request, "Updated advisor quick link ", $data);
            }
    ...
    public function archive(Request $request){
        ...
        $this->saveActivity($request, "Deleted advisor quick link ");
        return $this->output();
    }
}
