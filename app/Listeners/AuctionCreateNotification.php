...
class AuctionCreateNotification
{
    ...
    {
        if($event->active_buy_out){
            $this->sendActiveButOutMail($event);
        }else{
            $this->sendRegularMail($event);
        }   
    }
     ...
     * Send Regular Mail
     */
    public function sendRegularMail($event){
     ...
     */
    public function sendActiveButOutMail($event){
        $auction = $event->auction;
