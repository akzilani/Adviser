

<div class="modal-content" id="modal_1"">            
    <div class="modal-header">
        <h5 class="modal-title" > {{ $title ?? "Create/Edit"}} </h5>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
            <span aria-hidden="true">&times;</span>
        </button>
    </div> 
    {!! Form::open(['url'=> $form_url, 'method' => 'post', 'files' => 'true','class'=>'ajax-form']) !!}
        <div class="modal-body">
            <div class="row">
                <!-- Name -->
                <div class="col-12">
                    <div class="form-group">
                        <label>Name<span class="text-danger">*</span> </label>
                        <input type="hidden"  name="id" value="{{ $data->id ?? 0 }}" >
                        <input type="text"  name="name" value="{{ $data->name ?? "" }}" class="form-control" required >
                    </div>
                </div>

                 <!-- Name -->
                 <div class="col-12">
                    <div class="form-group">
                        <label>Location </label>
                        <input type="text"  name="location" value="{{ $data->location ?? "" }}" class="form-control" >
                    </div>
                </div>

                <!-- Publication Status -->
                <div class="col-12">
                    <div class="form-group">
                        <label>Publication Status <span class="text-danger">*</span></label>
                        <select name="publication_status" class="form-control" required >
                            <option>Select Publication Status</option>
                            <option value="1" {{ isset($data->id) && $data->publication_status ? 'selected' : Null }}>Published</option>
                            <option value="0" {{ isset($data->id) && !$data->publication_status ? 'selected' : Null }} >Unpublished</option>                           
                        </select>
                    </div>
                </div> 

                <!-- Description -->
                <div class="col-12">
                    <div class="form-group">
                        <label>Description<span class="text-danger">*</span> </label>
                        <textarea name="description" class="form-control" required >{{ $data->description ?? Null }}</textarea>
                    </div>
                </div>
                
                <!--
                <div class="col-12 ">
                    <div class="form-group" >
                      <label class="checkbox-inline" style="display:inline-block;"> <input type="checkbox" id="i-agree_btn" required>    {!! $terma_and_condition->trems_and_condition ?? "" !!}  </label>
                    </div>
                </div>
                -->
                
                
                <div class="col-12 ">
                    <div class="form-group form-check">
                        <input type="checkbox" class="form-check-input" id="exampleCheck1" required>
                        <label class="form-check-label" for="exampleCheck1" style="padding-left:5px;"> {!! $terma_and_condition->trems_and_condition ?? "" !!}</label>
                    </div>
                </div>


                <div class="col-12 col-sm-6">
                    <label>Loading</label>
                    <div class="progress">
                        <div class="progress-bar progress-bar-striped" role="progressbar" style="width: 50%" aria-valuenow="50" aria-valuemin="0" aria-valuemax="100"> 0% </div>
                    </div> 
                </div>
            </div>            
            <div class="modal-footer">
                <button type="button" class="btn btn-sm btn-danger float-left" data-dismiss="modal" visible="false">Close</button>
                <button type="submit" name="btn" class="btn btn-sm btn-primary" visible="false"> Save </button>
            </div>
        </div>
    {!! Form::close() !!}
</div>


<!--

    <div class="modal" id="trems_and_condition" data-backdrop="static">
        <div class="modal-dialog">
            <div class="modal-content modal-lg">
                <div class="modal-header text-left">                    
                    <h4 class="modal-title text-left">Testimonial Terms and Condition</h4>
                </div>
                <div class="container">
                </div>
                <div class="modal-body">
                    {!! $terma_and_condition->trems_and_condition ?? "" !!}
                </div>
                <div class="modal-footer">
                    <button id="agree_btn" class="btn btn-success">I Agree</button>
                </div>
            </div>
        </div>
    </div>

    <script type="text/javascript">
        $(document).on("change", "#i-agree_btn", function(){
            $("#modal_1").addClass("d-none");
            $("#trems_and_condition").modal("show");
        });

        $(document).on("click", "#agree_btn", function(){
            $("#trems_and_condition").modal("hide");
            $("#modal_1").removeClass("d-none");
        })
    </script>

-->

    
    